...
function [Y_new, votes, prediction_per_tree,proximity_ts,nodes] = classRF_predict(X,model, extra_options)
    ...
    maxcat = max(ncat);
	[Y_hat,prediction_per_tree,votes,proximity_ts] = mexClassRF_predict(X',model.nrnodes,model.ntree,model.xbestsplit,model.classwt,model.cutoff,model.treemap,model.nodestatus,model.nodeclass,model.bestvar,model.ndbigtree,model.nclass, predict_all, proximity, nodes, int32(ncat), maxcat );
	%keyboard
    votes = votes';
