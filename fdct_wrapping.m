...
% figure,imshow(log(1+abs(X)),[])
% figure,imshow(log(1+abs(X(Xlow_index_1, Xlow_index_2))),[])
% figure,imshow(log(1+abs(Xhi)),[])
for j = scales,
   ...
   % figure,imshow(log(1+abs(Xlow)),[])
    ...
    for quadrant = 1:nbquadrants
        ...
        %figure,imshow(log(1+abs(wrapped_data)),[])
