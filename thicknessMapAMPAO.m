function thicknessMapAMPAO(boundaryPoints,header,circ_rad,showgrid,handles)
...
% % rotate the thickness maps to have an aligned SLO
% thicknessVals_rot = imrotate(thicknessVals, handles.data.sloRotation);
% [size_thick_x size_thick_y] = size(thicknessVals_rot);
% stepx = fix((size_thick_x-512)/2);
% stepy = fix((size_thick_y-512)/2);
% 
% thicknessVals = thicknessVals_rot(stepx+1:size_thick_x-stepx-1,stepy+1:size_thick_y-stepy-1);
...
%     hold on
    ...
    axis off
    colorbar
    title('Thickness values by ETDRS grid sector')
