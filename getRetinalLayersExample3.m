...
if isUseExampleImage    
    ...
    for i=[1,5,3]
        counti = counti +1;
        mat2tiff(d5(:,:,i),folderPath, strcat('imi',num2str(i)))
        filename{counti} = strcat('imi',num2str(i),'.tiff');
    end
    

%     path = '\';
%     [filename, folderPath , filterindex] = uigetfile([path '*.tiff' ],'Pick some images','MultiSelect', 'on');
    filterindex = 2;
